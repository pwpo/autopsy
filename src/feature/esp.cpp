...
namespace style
{
    ...
    static void hat(ImDrawList* draw, const sdk::part& head)
    {
        ...
        std::vector<ImVec2> brim;
        brim.reserve(Segments);
        ...
        draw->AddConvexPolyFilled(brim.data(), (int)brim.size(), fromfloat(global::esp::color::hat, .075f));
        ...
        draw->AddPolyline(brim.data(), (int)brim.size(), shadow, true, 3.4f);
        draw->AddPolyline(brim.data(), (int)brim.size(), alpha(Purple, .30f), true, 2.05f);
        draw->AddPolyline(brim.data(), (int)brim.size(), accent, true, 1.2f);
...
namespace esp {
    ...
    void run()
    {
        ...
        for (auto& player : Snapshot)
        {
            ...
            auto Head_W2S = global::render.screen(Head.primitive().position());
            ...
            if (global::esp::Trails)
            {
                ...
                if (style::trailanchor(player, TrailAnchor))
            ...
            if (global::esp::Skeleton) {
                const ImU32 SkelCol = FlotationDevice(global::esp::color::Skeleton);
                ...
                auto DrawPoly = [&](const ImVec2* Points, int Count) {
                    ...
                    Draw->AddPolyline(Points, Count, style::alpha(style::Purple, .35f), false, Thickness + 1.2f);
        ...
        auto DrawFovCircle = [&](bool ShouldDraw, float FovRadius, const float tocolor[4], bool Fill, bool Spin, int SpinSpeed)
            {
                ...
                FovDraw->Flags |= ImDrawListFlags_AntiAliasedLines | ImDrawListFlags_AntiAliasedFill;
                style::fov(FovDraw, MousePos, FovRadius, tocolor, Fill, Spin, SpinSpeed);
            };
