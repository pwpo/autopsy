...
{
    return drive->read<std::uint64_t>(BaseAddress + offset::mouseservice::InputObject);
}
...
void silent::frame() {
    ...
    for (;;) {
        ...
        mouseservice = std::make_unique<sdk::instance>(global::model.childclass("MouseService"));
        silentkey();
        ...
        if (!SilentFTarget || SilentCachedTarget.character.Address == 0) {
            ...
            sdk::instance TargetPart = targetpart(Target, global::silent::AimPart);
            SilentFTarget = (TargetPart.Address != 0);
            SilentCachedTarget = Target;
        }
        else {
            if (!global::silent::StickyAim) {
                Target = closestplayer();
                SilentFTarget = (Target.character.Address != 0);
                SilentCachedTarget = Target;
            }
            else if (global::silent::UseFov) {
                if (!targetfov(SilentCachedTarget)) {
                    SilentFTarget = false;
                    SilentCachedTarget = {};
                    continue;
                }
        ...
        if (SilentFTarget && SilentCachedTarget.character.Address != 0) {
            if (global::silent::KnockedCheck && knocked(SilentCachedTarget)) {
                SilentFTarget = false;
                SilentCachedTarget = {};
                SilentPrimitiveActive = false;
                continue;
            ...
            if (TargetPart.Address != 0) {
                if (camerasilent(SilentCachedTarget)) {
                    SilentPrimitiveActive = true;
                    IsSilentReady = false;
